...
const TvAndHomePage = ({ isAuthenticated }) => {
    const [tvAndHomeProducts, setTvAndHomeProducts] = useState([]);
    ...
    useEffect(() => {
        async function fetchTvAndHomeProducts() {
            try {
                const { data } = await axios.get('http://127.0.0.1:8000/api/getProducts/');
                const tvAndHomeProducts = data.filter(product => product.category === 'tvandhome');
                setTvAndHomeProducts(tvAndHomeProducts);
            } catch (error) {
                console.error("Error fetching TV and Home products:", error);
            }
        }
        fetchTvAndHomeProducts();
    }, []);

    return (
        <div className="d-flex flex-column" style={{ minHeight: "100vh" }}>
            <div className="container py-5">
                <h1 className="mb-4">Apple TV and Home Products</h1>
                <Row>
                    {tvAndHomeProducts.map((product, index) => (
