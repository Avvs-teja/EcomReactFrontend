...
const IPadPage = ({ isAuthenticated }) => {
    ...
    useEffect(() => {
        async function fetchIPadProducts() {
            try {
                ...
                const iPadProducts = data.filter(product => product.category === 'ipad');
                setIPadProducts(iPadProducts);
            } catch (error) {
    ...
    return (
        <div className="d-flex flex-column" style={{ minHeight: "100vh" }}>
            <div className="container py-5">
                <h1 className="mb-4">Apple iPad's</h1>
                <Row>
