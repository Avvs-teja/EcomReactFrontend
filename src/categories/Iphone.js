...
const Iphone = ({ isAuthenticated }) => {
    ...
    useEffect(() => {
        async function fetchIphoneProducts() {
            try {
                const { data } = await axios.get('http://127.0.0.1:8000/api/getProducts/');
                const iphoneProducts = data.filter(product => product.category === 'iphone');
                setIphoneProducts(iphoneProducts);
            } catch (error) {
    ...
    return (
        <div className="d-flex flex-column" style={{ minHeight: "100vh" }}>
            <div className="container py-5">
                <h1 className="mb-4">Apple iPhone's</h1>
                <Row>
