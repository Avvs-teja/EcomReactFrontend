...
const OthersPage = ({ isAuthenticated }) => {
    ...
    useEffect(() => {
        async function fetchOthersProducts() {
            try {
                const { data } = await axios.get('http://127.0.0.1:8000/api/getProducts/');
                const othersProducts = data.filter(product => product.category === 'others');
                setOthersProducts(othersProducts);
            } catch (error) {
    ...
    return (
        <div className="d-flex flex-column" style={{ minHeight: "100vh" }}>
            <div className="container py-5">
                <h1 className="mb-4">Apple Other Products</h1>
                <Row>
